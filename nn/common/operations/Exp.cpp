...
constexpr char kOperationName[] = "EXP";
...
inline bool compute(const T* input, const Shape& shape, T* output) {
    ...
    for (uint32_t i = 0; i < size; ++i) {
        output[i] = static_cast<T>(std::exp(static_cast<float>(input[i])));
    }
    return true;
}
...
NN_REGISTER_OPERATION(EXP, exp::kOperationName, exp::validate, exp::prepare, exp::execute);
