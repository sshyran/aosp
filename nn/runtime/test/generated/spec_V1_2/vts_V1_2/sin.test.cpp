...
TEST_F(DynamicOutputShapeTest, sin_relaxed_dynamic_output_shape) {
  Execute(device,
          createTestModel_relaxed_dynamic_output_shape,
          is_ignored_relaxed_dynamic_output_shape,
          ::generated_tests::sin::get_examples_relaxed_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, sin_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_relaxed_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::sin::get_examples_relaxed_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, sin_float16_dynamic_output_shape) {
  Execute(device,
          createTestModel_float16_dynamic_output_shape,
          is_ignored_float16_dynamic_output_shape,
          ::generated_tests::sin::get_examples_float16_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, sin_float16_dynamic_output_shape) {
  const Model model = createTestModel_float16_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::sin::get_examples_float16_dynamic_output_shape());
  validateEverything(model, requests);
}
