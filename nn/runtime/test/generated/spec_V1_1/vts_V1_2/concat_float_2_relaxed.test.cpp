...
TEST_F(DynamicOutputShapeTest, concat_float_2_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::concat_float_2_relaxed::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, concat_float_2_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::concat_float_2_relaxed::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, concat_float_2_relaxed_all_inputs_as_internal_dynamic_output_shape) {
  Execute(device,
          createTestModel_all_inputs_as_internal_dynamic_output_shape,
          is_ignored_all_inputs_as_internal_dynamic_output_shape,
          ::generated_tests::concat_float_2_relaxed::get_examples_all_inputs_as_internal_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, concat_float_2_relaxed_all_inputs_as_internal_dynamic_output_shape) {
  const Model model = createTestModel_all_inputs_as_internal_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::concat_float_2_relaxed::get_examples_all_inputs_as_internal_dynamic_output_shape());
  validateEverything(model, requests);
}
