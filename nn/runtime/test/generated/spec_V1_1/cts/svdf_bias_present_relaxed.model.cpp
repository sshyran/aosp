...
void CreateModel_all_inputs_as_internal(Model *model) {
  ...
  model->addOperation(ANEURALNETWORKS_ADD, {state_in_tmp, dummy4, param4}, {state_in});
  model->addOperation(ANEURALNETWORKS_SVDF, {input, weights_feature, weights_time, bias, state_in, rank_param, activation_param}, {state_out, output});
  // Phase 3, inputs and outputs
  model->identifyInputsAndOutputs(
    {input_tmp, weights_feature_tmp, weights_time_tmp, bias_tmp, state_in_tmp},
    {state_out, output});
  // Phase 4: set relaxed execution
  model->relaxComputationFloat32toFloat16(true);
  assert(model->isValid());
...
void CreateModel_all_inputs_as_internal_dynamic_output_shape(Model *model) {
  ...
  model->addOperation(ANEURALNETWORKS_ADD, {weights_feature_tmp, dummy6, param6}, {weights_feature});
  model->addOperation(ANEURALNETWORKS_ADD, {weights_time_tmp, dummy7, param7}, {weights_time});
  model->addOperation(ANEURALNETWORKS_ADD, {bias_tmp, dummy8, param8}, {bias});
  model->addOperation(ANEURALNETWORKS_ADD, {state_in_tmp, dummy9, param9}, {state_in});
  model->addOperation(ANEURALNETWORKS_SVDF, {input, weights_feature, weights_time, bias, state_in, rank_param, activation_param}, {state_out, output});
  // Phase 3, inputs and outputs
  model->identifyInputsAndOutputs(
    {input_tmp, weights_feature_tmp, weights_time_tmp, bias_tmp, state_in_tmp},
