...
TEST_F(DynamicOutputShapeTest, l2_pool_float_large_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::l2_pool_float_large::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, l2_pool_float_large_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::l2_pool_float_large::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, l2_pool_float_large_all_inputs_as_internal_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::l2_pool_float_large::get_examples_all_inputs_as_internal_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, l2_pool_float_large_all_inputs_as_internal_dynamic_output_shape) {
  const Model model = createTestModel_all_inputs_as_internal_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::l2_pool_float_large::get_examples_all_inputs_as_internal_dynamic_output_shape());
  validateEverything(model, requests);
}
