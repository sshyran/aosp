...
TEST_F(DynamicOutputShapeTest, add_broadcast_quant8_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::add_broadcast_quant8::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, add_broadcast_quant8_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::add_broadcast_quant8::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, add_broadcast_quant8_all_inputs_as_internal_dynamic_output_shape) {
  Execute(device,
          createTestModel_all_inputs_as_internal_dynamic_output_shape,
          is_ignored_all_inputs_as_internal_dynamic_output_shape,
          ::generated_tests::add_broadcast_quant8::get_examples_all_inputs_as_internal_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, add_broadcast_quant8_all_inputs_as_internal_dynamic_output_shape) {
  const Model model = createTestModel_all_inputs_as_internal_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::add_broadcast_quant8::get_examples_all_inputs_as_internal_dynamic_output_shape());
  validateEverything(model, requests);
}
