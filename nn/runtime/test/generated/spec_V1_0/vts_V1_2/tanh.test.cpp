...
namespace android::hardware::neuralnetworks::V1_2::generated_tests::tanh {
...
TEST_F(DynamicOutputShapeTest, tanh_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::tanh::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, tanh_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::tanh::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, tanh_all_inputs_as_internal_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::tanh::get_examples_all_inputs_as_internal_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, tanh_all_inputs_as_internal_dynamic_output_shape) {
  const Model model = createTestModel_all_inputs_as_internal_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::tanh::get_examples_all_inputs_as_internal_dynamic_output_shape());
  validateEverything(model, requests);
}
