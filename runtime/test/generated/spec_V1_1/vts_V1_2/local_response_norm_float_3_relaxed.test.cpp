...
TEST_F(NeuralnetworksHidlTest, local_response_norm_float_3_relaxed) {
  Execute(device,
          createTestModel,
          is_ignored,
          ::generated_tests::local_response_norm_float_3_relaxed::get_examples());
}

TEST_F(ValidationTest, local_response_norm_float_3_relaxed) {
  const Model model = createTestModel();
  const std::vector<Request> requests = createRequests(::generated_tests::local_response_norm_float_3_relaxed::get_examples());
...
TEST_F(DynamicOutputShapeTest, local_response_norm_float_3_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::local_response_norm_float_3_relaxed::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, local_response_norm_float_3_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::local_response_norm_float_3_relaxed::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
