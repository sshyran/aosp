...
TEST_F(DynamicOutputShapeTest, mobilenet_224_gender_basic_fixed_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::mobilenet_224_gender_basic_fixed_relaxed::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, mobilenet_224_gender_basic_fixed_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::mobilenet_224_gender_basic_fixed_relaxed::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
