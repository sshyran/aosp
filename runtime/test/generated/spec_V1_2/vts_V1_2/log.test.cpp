...
TEST_F(DynamicOutputShapeTest, log_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::log::get_examples_relaxed_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, log_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_relaxed_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::log::get_examples_relaxed_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, log_float16_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::log::get_examples_float16_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, log_float16_dynamic_output_shape) {
  const Model model = createTestModel_float16_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::log::get_examples_float16_dynamic_output_shape());
  validateEverything(model, requests);
}
