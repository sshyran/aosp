...
TEST_F(DynamicOutputShapeTest, bbox_graph_zero_sized_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::bbox_graph::get_examples_zero_sized_relaxed_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, bbox_graph_zero_sized_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_zero_sized_relaxed_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::bbox_graph::get_examples_zero_sized_relaxed_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, bbox_graph_zero_sized_float16_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::bbox_graph::get_examples_zero_sized_float16_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, bbox_graph_zero_sized_float16_dynamic_output_shape) {
  const Model model = createTestModel_zero_sized_float16_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::bbox_graph::get_examples_zero_sized_float16_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, bbox_graph_zero_sized_quant8_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::bbox_graph::get_examples_zero_sized_quant8_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, bbox_graph_zero_sized_quant8_dynamic_output_shape) {
  const Model model = createTestModel_zero_sized_quant8_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::bbox_graph::get_examples_zero_sized_quant8_dynamic_output_shape());
  validateEverything(model, requests);
}
