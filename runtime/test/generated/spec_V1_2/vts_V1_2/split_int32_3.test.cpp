...
TEST_F(DynamicOutputShapeTest, split_int32_3_relaxed_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::split_int32_3::get_examples_relaxed_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, split_int32_3_relaxed_dynamic_output_shape) {
  const Model model = createTestModel_relaxed_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::split_int32_3::get_examples_relaxed_dynamic_output_shape());
  validateEverything(model, requests);
}
