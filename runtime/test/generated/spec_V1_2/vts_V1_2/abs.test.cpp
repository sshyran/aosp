...
TEST_F(DynamicOutputShapeTest, abs_dynamic_output_shape_relaxed) {
  Execute(device,
          ...
          ::generated_tests::abs::get_examples_dynamic_output_shape_relaxed(), true);
}

TEST_F(ValidationTest, abs_dynamic_output_shape_relaxed) {
  const Model model = createTestModel_dynamic_output_shape_relaxed();
  const std::vector<Request> requests = createRequests(::generated_tests::abs::get_examples_dynamic_output_shape_relaxed());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, abs_dynamic_output_shape_float16) {
  Execute(device,
          ...
          ::generated_tests::abs::get_examples_dynamic_output_shape_float16(), true);
}

TEST_F(ValidationTest, abs_dynamic_output_shape_float16) {
  const Model model = createTestModel_dynamic_output_shape_float16();
  const std::vector<Request> requests = createRequests(::generated_tests::abs::get_examples_dynamic_output_shape_float16());
  validateEverything(model, requests);
}
