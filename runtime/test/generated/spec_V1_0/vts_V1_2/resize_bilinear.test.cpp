...
TEST_F(DynamicOutputShapeTest, resize_bilinear_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::resize_bilinear::get_examples_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, resize_bilinear_dynamic_output_shape) {
  const Model model = createTestModel_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::resize_bilinear::get_examples_dynamic_output_shape());
  validateEverything(model, requests);
}
...
TEST_F(DynamicOutputShapeTest, resize_bilinear_all_inputs_as_internal_dynamic_output_shape) {
  Execute(device,
          ...
          ::generated_tests::resize_bilinear::get_examples_all_inputs_as_internal_dynamic_output_shape(), true);
}

TEST_F(ValidationTest, resize_bilinear_all_inputs_as_internal_dynamic_output_shape) {
  const Model model = createTestModel_all_inputs_as_internal_dynamic_output_shape();
  const std::vector<Request> requests = createRequests(::generated_tests::resize_bilinear::get_examples_all_inputs_as_internal_dynamic_output_shape());
  validateEverything(model, requests);
}
