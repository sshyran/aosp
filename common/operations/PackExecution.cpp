...
bool packParams(IOperationExecutionContext* context, tflite::PackParams* params) {
    ...
    NN_RET_CHECK_LE(axis, std::numeric_limits<__typeof__(params->axis)>().max())
            << "axis value out of range";
    params->axis = axis;

    const uint32_t inputTensorCount = context->getNumInputs() - 1;
    NN_RET_CHECK_LE(inputTensorCount, std::numeric_limits<__typeof__(params->inputs_count)>().max())
